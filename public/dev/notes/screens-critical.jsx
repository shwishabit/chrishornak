...
function MorningAnchor({ onMood, onMeditate, onBreaths, onReflect, onEnter, dateStr, weekday, momentum, regulars, onAddRegular, completion, todayLog }) {
  ...
  return (
    // v=34: bottom padding extended (32 → 60) to clear the pinned horizon
    // strip (~26px) without crowding the Open-today button.
    <div className="screen fade-soft" style={{justifyContent: "space-between", padding: "56px 32px 60px", position: "relative"}}>
...
// ===== v=34: horizon space (Wins + Goals) =====
//
// A parallel environment for non-daily long-term thinking. Wins (backward
// recognition) + Goals (forward intent) merged as spontaneous-thoughts
// siblings. Same TabBar, same gestures (tap-to-open drawer), same vocabulary
// — only the paper tone shifts (paper-cream → paper-dusk via [data-mode=
// "horizon"] on .phone-frame). Day-flip plumbing doesn't apply: markers,
// 3am boundary, Recap+Carry, recurrences are all Daily Now plumbing only.
//
// Active goals are soft-capped at 3 (mirrors Active Frame's soft-cap-at-10
// pattern in NowPage). Goals migrate the spectrum: active → desk-top
// → desk-back (drawer) → trash. Wins are timeline-anchored, no migration.
//
// Critical-path placement (vs deferred): tapping the horizon strip should
// feel instant. Deferring would show DeferredFallback for ~100-300ms on
// the very first flip, which contradicts "felt mode shift." The bundle cost
// is acceptable — these screens are leaner than NowPage / DeskPage.

// ---------- Horizon strip ----------
// Persistent visible affordance for entering/leaving horizon mode. Renders
// above TabBar on screens with TabBar; pinned to bottom of phone-frame on
// Anchor (no TabBar). Suppressed entirely on modal flows by App's render
// gate, not by this component.
function HorizonStrip({ horizonMode, onToggle, pinned }) {
  return (
    <button
      type="button"
      onClick={onToggle}
      className={`horizon-strip ${pinned ? "horizon-strip--pinned" : ""}`}
      aria-label={horizonMode ? "return to today" : "open horizon"}
      style={{
        background: "var(--paper-deep)",
        border: "none",
        width: "100%",
    ...
    >
      <span>{horizonMode ? "today ↓" : "horizon"}</span>
    </button>
  );
}

// ---------- Horizon overview ----------
// The "begin" landing page when in horizon mode. Active goals + recent wins
// glance — a quiet reading surface, not an action surface. Adds happen from
// the matching tabs (Goals notebook / Wins timeline). Empty-state philosophy
// (per V2 design): empty isn't a problem to solve. The room being patient is
// itself the design.
function HorizonOverview({ goals, wins, dateStr, weekday, onOpenGoals, onOpenWins }) {
  const active = goals.filter(g => g.tier === "active");
  const recentWins = wins.slice(0, 5);

  return (
    // Bottom padding accommodates the pinned horizon strip (~26px) below.
    <div className="screen fade-soft" style={{padding: "56px 28px 60px", overflowY: "auto"}}>
      <div style={{textAlign: "left"}} className="ascend">
        <div className="kicker" style={{marginBottom: 10}}>{weekday}</div>
        <div className="serif" style={{fontSize: 20, color: "var(--ink-soft)", letterSpacing: "0.01em"}}>
          horizon
        </div>
        <div className="serif" style={{fontSize: 13, color: "var(--ink-faint)", fontStyle: "italic", marginTop: 4}}>
          a future thinking space.
        </div>
      </div>

      <div className="ascend" style={{marginTop: 36, animationDelay: "120ms"}}>
        <button
          onClick={onOpenGoals}
          style={{
            width: "100%",
        ...
        >
          <div className="kicker" style={{color: "var(--ink-faint)", marginBottom: 4}}>active</div>
        </button>
        {active.length === 0 ? (
          <div className="serif" style={{
            fontSize: 14, fontStyle: "italic",
            color: "var(--ink-faint)", padding: "8px 0 18px",
            lineHeight: 1.55,
          }}>
      ...
      <div className="ascend" style={{marginTop: 28, animationDelay: "200ms"}}>
        <button
          onClick={onOpenWins}
          style={{
            width: "100%",
        ...
        >
          <div className="kicker" style={{color: "var(--ink-faint)", marginBottom: 4}}>recent wins</div>
        </button>
        {recentWins.length === 0 ? (
          <div className="serif" style={{
            fontSize: 14, fontStyle: "italic",
...
// ---------- Goals notebook (active goals) ----------
// Mirrors NowPage shape but lean. Active goals only. Soft-cap 3 with a
// quiet color shift at 4+. No D&D, no marker ladder, no progress, no
// momentum. Add via FAB → AddGoalSheet. Drawer wraps row interactions.
function GoalsNotebook({ goals, tasks, onAddOpen, onRename, onSetContext, onMove, onRelease, dateStr, weekday }) {
  const [openId, setOpenId] = useState(null);
  const active = goals.filter(g => g.tier === "active");
  const overCap = active.length > 3;

  // For each active goal, count tasks in today's Notebook that point here.
  // Read-only descriptive line per V2 design — never evaluative, no nag.
  const refCounts = (() => {
    const counts = {};
  ...
  return (
    <div className="screen surface-notebook" style={{padding: "12px 0 0", overflowY: "auto"}}>
      <div className="surface-mark" aria-hidden="true"/>
      <div style={{padding: "12px 28px 18px", display: "flex", justifyContent: "space-between", alignItems: "flex-start"}}>
        <div>
          <div className="kicker" style={{marginBottom: 4}}>{weekday}</div>
...
// ---------- Goals desk ----------
// Two zones: top of desk ("worth considering") and back of desk = drawer
// ("less vital but not killed"). Mirrors DeskPage's layout pattern.
function GoalsDesk({ goals, onRename, onSetContext, onMove, onRelease, dateStr, weekday }) {
  const [openId, setOpenId] = useState(null);
  const top = goals.filter(g => g.tier === "desk-top");
  const back = goals.filter(g => g.tier === "desk-back");

  return (
    <div className="screen surface-desk" style={{padding: "12px 0 0", overflowY: "auto"}}>
      <div style={{padding: "18px 28px 18px"}}>
        <div className="kicker" style={{marginBottom: 4}}>{weekday}</div>
        <div className="serif" style={{fontSize: 28, fontWeight: 400, letterSpacing: "-0.01em", color: "var(--ink)"}}>
          Desk.
        </div>
        <div className="serif" style={{fontSize: 13, color: "var(--ink-faint)", marginTop: 2, fontStyle: "italic"}}>
          stepped back, not gone
        </div>
      </div>
      ...
      <div style={{padding: "16px 28px 6px"}}>
        <div className="kicker" style={{color: "var(--ink-faint)"}}>top of desk</div>
      </div>
      {top.length === 0 ? (
        <div className="serif" style={{padding: "0 28px 16px", fontSize: 13, fontStyle: "italic", color: "var(--ink-faint)"}}>
          empty.
        </div>
      ) : top.map(g => (
        <GoalRow
      ...
      <div style={{padding: "20px 28px 6px"}}>
        <div className="kicker" style={{color: "var(--ink-faint)"}}>drawer</div>
      </div>
      {back.length === 0 ? (
        <div className="serif" style={{padding: "0 28px 16px", fontSize: 13, fontStyle: "italic", color: "var(--ink-faint)"}}>
          empty.
...
// ---------- Goals trash ----------
// Released goals. Restore (back to active) or delete forever. Resurface
// band (Low/Mid/High per 2026-04-27 design) deferred to a future ship —
// v=34 keeps trash linear so the spectrum's first surface is small enough
// to evaluate without compounded mechanics.
function GoalsTrash({ goals, onRestore, onPurge, onMove, onRename, onSetContext, dateStr, weekday }) {
  const [openId, setOpenId] = useState(null);
  const released = goals.filter(g => g.tier === "trash");

  return (
    <div className="screen" style={{padding: "12px 0 0", overflowY: "auto"}}>
      <div style={{padding: "18px 28px 18px"}}>
        <div className="kicker" style={{marginBottom: 4}}>{weekday}</div>
        <div className="serif" style={{fontSize: 28, fontWeight: 400, letterSpacing: "-0.01em", color: "var(--ink)"}}>
          Trash.
...
// ---------- Wins timeline ----------
// Persistent reverse-chronological scroll. No fade, no scroll-back limit.
// Manual retire only via row × button. A win logged six months ago is still
// a win — recognition events don't decay.
function WinsTimeline({ wins, goals, onRetire, dateStr, weekday }) {
  // Group by ISO day for visual rhythm. Same-day wins cluster under one
  // date label; across days, a thin rule visually separates groups.
  const groups = (() => {
    const out = [];
    let lastDay = null;
    for (const w of wins) {
...
Object.assign(window, {
  ...
  // v=34: horizon space
  HorizonStrip, HorizonOverview,
  GoalRow, GoalsNotebook, AddGoalSheet, GoalsDesk, GoalsTrash, WinsTimeline,
});
